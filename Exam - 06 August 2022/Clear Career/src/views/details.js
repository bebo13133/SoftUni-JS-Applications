...
import { apply, getApplication, getUserApplication } from "../data/applications.js"
...
const detailsTemplate = (offer,onDelete,onApply) => html`
      <section id="details">
          <div id="details-wrapper">
            ...
            <!--  -->
            ${offer.canApply?html`<div id="action-buttons">
                <a @click=${onApply} href="javascript:void(0)" id="apply-btn">Apply</a></div>`:null}
          </div>
        </section>`
export async function detailsPage(ctx) {
    ...
    const request = [
        ...
        const [offer, applications, hasApply] = await Promise.all(request)
        offer.applications = applications

    if(userData && userData._id == offer._ownerId){
        offer.canEdit = true
    }
    if(userData && userData._id !== offer._ownerId && hasApply==0){
        offer.canApply=true
    }
